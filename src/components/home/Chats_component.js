...
import { setReciever, setRecieverData, setFriends, readAllMessageOf } from "../../reducers/chatReducer";

function Chats_component(props) {
  // const [myData, setMyData] = useState([]);

  const { recieverId, friends } = useSelector((state) => state.chat);
  ...
  useEffect(() => {
    axios.get(`${API}/friends`, {
      ...
      dispatch(setFriends(res.data[0]?.friends || []));
    }).catch((err) => {
      console.log(err);
    })
  }, [])
  ...
  const handleChangeChatUser = (chat_item) => {
    dispatch(setReciever(chat_item?.friend?._id));
    dispatch(setRecieverData(chat_item?.friend));
    dispatch(readAllMessageOf({ fid: chat_item?.friend?._id }));
  }

  useEffect(() => {
    console.log("friends changes");
  }, [friends])

  return (
    <div>
      <div className="">
        {myData.map((chat_item, key) => {
          return (
            <div
              key={key}
              className={`${recieverId === chat_item?.friend?._id && "bg-gray-200 "} h-[72px] cursor-pointer w-full flex items-center pr-[8px] hover:bg-[#F0F2F5] hover:transition-all`}
              onClick={() => handleChangeChatUser(chat_item)}
            >
                ...
                <div className="h-[42px] w-full pr-[10px]">
                  <div className="h-[22px] flex items-center justify-between">
                    <div className="text-[#111b21] text-[17px]">
                      {chat_item?.friend?.name}
                    </div>
                    <div className="text-[12px] text-[#667781]">
                      {chat_item?.last_message?.time || "12:10 PM"}
                    </div>
                  </div>

                  <div className="h-[20px] flex w-full justify-between ">
                    ...
                    {chat_item?.unreadMessage > 0 && (
                      <div className="bg-red-700 text-white flex items-center justify-center rounded-[50px] w-[22px] h-[22px] text-sm box-border">
                        {chat_item?.unreadMessage}
                      </div>
                    )}
                  </div>
                </div>
