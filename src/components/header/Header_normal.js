  ...
  return (
    <div>
        ...
        <div className="flex items-center grow shrink" onClick={()=>changeIsProfileOpen(true)}>
            { reciever.dp ? (<div className="rounded-full h-[40px] w-[40px] bg-cover bg-center mr-4 cursor-pointer" style={{ backgroundImage:`url(${reciever.dp})` }} title="Profile Details"></div>) : (<div className='h-[40px] w-[40px] rounded-full mr-4'>{contactIconDefault}</div>) }
            <div className="h-[40px] flex flex-col cursor-pointer">
                <p className="text-[16px] leading-[21px] text-white/90 capitalize">{reciever?.name}</p>
